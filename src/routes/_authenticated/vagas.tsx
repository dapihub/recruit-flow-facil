...
const ETAPA_COLORS: Record<string, string> = {
  ...
  "Contrato": "bg-brand/15 text-brand",
  "Descritivo publicado": "bg-purple-500/15 text-purple-600",
  "Candidatos em triagem": "bg-warning/20 text-warning-foreground",
  "Em Garantia": "bg-orange-500/15 text-orange-600",
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              ...
              <tbody className="divide-y divide-border/50">
                {filtradas.map((v) => (
                  <tr key={v.id} onClick={() => navigate({ to: "/vagas/$vagaId", params: { vagaId: v.id } })}
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent onClick={(e) => e.stopPropagation()}>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>Esta ação não pode ser desfeita. A vaga "{v.cargo}" será removida permanentemente.</AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
                                onClick={async (e) => {
                                  e.stopPropagation();
                                  try { await deleteVaga(v.id); toast.success("Vaga excluída."); }
                                  catch (err: any) { toast.error(err?.message ?? "Não foi possível excluir a vaga."); }
...
const KPI_BAR: Record<string, string> = { brand: "bg-brand", success: "bg-success", warning: "bg-warning", info: "bg-info" };
  ...
  return (
    <DialogContent className="max-w-lg">
      ...
      <DialogFooter>
        ...
        <Button className="bg-brand hover:bg-brand/90 text-brand-foreground" disabled={!form.cargo || !form.empresa}
          onClick={async () => {
            try {
              await addVaga({ cargo: form.cargo, empresa: form.empresa, quantidade: Number(form.quantidade) || 1, receita: Number(form.receita) || 0, prazoGarantia: Number(form.prazoGarantia) || 90 });
              toast.success("Vaga criada e receita lançada no financeiro.");
              onClose();
            } catch { toast.error("Não foi possível salvar a vaga."); }
          }}>Criar vaga</Button>
      </DialogFooter>
    </DialogContent>
  );
...
function EditarVagaButton({ vaga }: { vaga: Vaga }) {
  ...
  const [form, setForm] = useState({
    cargo: vaga.cargo, empresa: vaga.empresa,
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => {
      setOpen(o);
      if (o) setForm({ cargo: vaga.cargo, empresa: vaga.empresa, quantidade: String(vaga.briefing?.quantidade ?? 1), prazoGarantia: String(vaga.prazoGarantia ?? 90), createdAt: vaga.createdAt ? vaga.createdAt.slice(0, 10) : "", prazo: vaga.prazo ? vaga.prazo.slice(0, 10) : "" });
    }}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-brand"><Pencil className="w-4 h-4" /></Button>
      </DialogTrigger>
      <DialogContent className="max-w-lg">
          ...
          <Button className="bg-brand hover:bg-brand/90 text-brand-foreground" disabled={!form.cargo || !form.empresa}
            onClick={async () => {
              try {
                ...
                await updateVaga(vaga.id, { cargo: form.cargo, empresa: form.empresa, prazoGarantia: Number(form.prazoGarantia) || 90, briefing: { ...(vaga.briefing ?? {} as any), quantidade: novaQtd }, createdAt: form.createdAt ? new Date(form.createdAt + "T12:00:00").toISOString() : vaga.createdAt, prazo: form.prazo || vaga.prazo });
