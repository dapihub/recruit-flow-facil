...
export function AppSidebar() {
  ...
  const candidatosTriagem = candidatos.filter((c) => c.status === "Triagem").length;
  const proximasAcoesVencidas = candidatos.filter((c) => c.proximaAcaoData && c.proximaAcaoData <= hoje && c.status !== "Contratado" && c.status !== "Reprovado").length;
  const temAlerta = faturasVencidas > 0 || candidatosTriagem > 0 || proximasAcoesVencidas > 0;
    ...
    <aside className="w-64 shrink-0 bg-[var(--sidebar-bg)] text-[var(--sidebar-fg)] flex flex-col h-screen sticky top-0 shadow-xl">
      ...
      <nav className="flex-1 p-4 space-y-1">
        {items.map(({ to, label, icon: Icon }) => {
          ...
          const showAlertDot = to === "/" && temAlerta;
          const badgeFinanceiro = to === "/financeiro" && faturasVencidas > 0;
          const badgeCandidatos = to === "/candidatos" && (candidatosTriagem > 0 || proximasAcoesVencidas > 0);
          const badgeCandidatosCount = candidatosTriagem + proximasAcoesVencidas;
        ...
        <div className="pt-2 border-t border-white/10 mt-2">
          {bottomItems.map(({ to, label, icon: Icon }) => {
            ...
            return (
              <Link key={to} to={to}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
                  active ? "bg-[var(--sidebar-active)] text-white shadow" : "text-white/75 hover:bg-white/5 hover:text-white"
                }`}>
      ...
      <div className="p-4 border-t border-white/10 space-y-3">
        ...
        <p className="text-[11px] text-white/40 pt-1 capitalize">{mesAtual}</p>
        <div className="pt-3 border-t border-white/10 space-y-2">
          <p className="text-[11px] text-white/60 truncate" title={user?.email ?? ""}>{user?.email}</p>
          <button
